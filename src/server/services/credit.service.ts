...
export async function getCreditPosition(
  ...
  const [invoices, payments, memos] = await Promise.all([
    ...
    prisma.creditMemo.aggregate({
      where: { customerId, status: { in: ['OPEN', 'APPLIED'] }, remainingAmount: { gt: 0 } },
...
export async function applyCreditMemo(
  ...
  const memo = await prisma.creditMemo.findFirst({
    ...
    select: {
      id: true, number: true, customerId: true, status: true, remainingAmount: true,
      saleId: true,
      customer: { select: { name: true } },
    },
  })
  if (!memo) throw notFound('That credit memo')
  if (memo.status === 'VOIDED') throw conflict(`${memo.number} has been voided.`)
  ...
  const openInvoices = await prisma.sale.findMany({
    where: { customerId: memo.customerId, status: 'COMPLETED', balanceDue: { gt: 0 } },
  ...
  const plan: { saleId: string; saleNumber: string; amount: ReturnType<typeof m> }[] = []
  ...
  await prisma.$transaction(async (tx) => {
    for (const entry of plan) {
      ...
      await tx.sale.update({
        ...
        data: {
          ...
          creditsApplied: toAmountString(m(sale.creditsApplied).plus(entry.amount)),
        },
      })
    }
    ...
    await writeAudit(tx, ctx, {
      ...
      after: {
        ...
        invoices: plan.map((p) => ({ saleNumber: p.saleNumber, amount: toAmountString(p.amount) })),
      },
    })
  })
...
export async function unapplyCreditMemo(
  ...
  const memo = await prisma.creditMemo.findFirst({
    ...
    select: {
      id: true, number: true, customerId: true, remainingAmount: true, status: true,
      applications: {
  ...
  const restored = memo.applications.reduce((total, a) => total.plus(m(a.amount)), m(0))
  ...
  await prisma.$transaction(async (tx) => {
    for (const application of memo.applications) {
      ...
      await tx.sale.update({
        where: { id: application.saleId },
        data: {
          balanceDue: toAmountString(m(sale.balanceDue).plus(application.amount)),
          creditsApplied: toAmountString(m(sale.creditsApplied).minus(application.amount)),
        },
      })
    }
...
export async function issueRefund(
  ...
  const replay = await prisma.refund.findFirst({
    ...
    select: {
      id: true, refundNumber: true, amount: true,
  ...
  const memo = await prisma.creditMemo.findFirst({
    where: { id: input.creditMemoId },
    select: {
      id: true, number: true, customerId: true, status: true,
  ...
  if (amount.lessThanOrEqualTo(0)) throw conflict('Enter how much is being refunded.')
  ...
  const created = await prisma.$transaction(async (tx) => {
    const refundNumber = await nextDocumentNumber(tx, ctx.organizationId, 'REFUND')
...
export async function voidRefund(
  ...
  const refund = await prisma.refund.findFirst({
    where: { id: refundId },
    select: {
      id: true, refundNumber: true, status: true, amount: true,
      creditMemo: { select: { id: true, number: true, remainingAmount: true, refundedAmount: true, status: true } },
    },
  })
  if (!refund) throw notFound('That refund')
  if (refund.status !== 'POSTED') throw conflict('That refund has already been voided.')
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (refund.creditMemo.status !== 'VOIDED') {
      await tx.creditMemo.update({
        ...
        data: {
          remainingAmount: toAmountString(m(refund.creditMemo.remainingAmount).plus(refund.amount)),
          refundedAmount: toAmountString(m(refund.creditMemo.refundedAmount).minus(refund.amount)),
          status: 'OPEN',
        },
      })
    }
...
): Promise<{ creditMemoId: string; number: string; amount: string; applied: string; remaining: string }> {
  requirePermission(ctx, 'credit:create')
  const prisma = db(ctx)

  const existing = await prisma.creditMemo.findFirst({
    where: { notes: { contains: input.idempotencyKey } },
  ...
  const memo = await prisma.$transaction(async (tx) => {
    ...
    const created = await tx.creditMemo.create({
      data: {
        ...
        // The idempotency key rides in the notes: an adjustment credit has no
        // natural key of its own, and a duplicate here is somebody's money.
        notes: [input.notes || null, `ref:${input.idempotencyKey}`].filter(Boolean).join(' · '),
...
export async function voidCreditMemo(
  ...
  const memo = await prisma.creditMemo.findFirst({
    where: { id: creditMemoId },
    select: {
      id: true, number: true, status: true, amount: true, remainingAmount: true,
  ...
  if (!memo) throw notFound('That credit memo')
  if (memo.status === 'VOIDED') throw conflict('That credit memo has already been voided.')
  ...
  if (memo.applications.length > 0) {
    throw conflict(`${memo.number} is applied to an invoice. Unapply it first.`)
  }
  if (memo.return) {
    throw conflict(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await writeAudit(tx, ctx, {
      ...
      entityType: 'CreditMemo',
      entityId: memo.id,
      after: { number: memo.number, amount: toAmountString(memo.amount), reason },
    })
  })
}
...
export async function snapshotParties(
  ...
  const [customer, issuer] = await Promise.all([
    prisma.customer.findFirstOrThrow({
      where: { id: customerId },
      select: {
        name: true, accountNumber: true, addressLine1: true, addressLine2: true,
