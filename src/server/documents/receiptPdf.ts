...
async function renderFullPage(doc: ReceiptDocument): Promise<Uint8Array> {
  ...
  page.drawText(doc.issuer.name, { x: margin, y: y - 16, size: 16, font: bold, color: INK })
  const heading = Number(doc.balanceDue) > 0 ? 'INVOICE' : 'RECEIPT'
  ...
  if (doc.void) {
    ...
    page.drawText(label, { x: margin + 12, y: y - 17, size: 13, font: bold, color: DANGER })
    const detail = [
      ...
      .filter(Boolean)
      .join(' · ')
    page.drawText(truncate(detail, font, 8, right - margin - 90), {
  ...
  y -= 14
  page.drawText(doc.billTo.name, { x: margin, y, size: 11, font: bold, color: INK })
  y -= 13
  ...
  if (doc.dueDate && Number(doc.balanceDue) > 0) {
    facts.push(['Due', formatDate(doc.dueDate, doc.timeZone)])
  }
  for (const [label, value] of facts) {
    page.drawText(label, { x: factsX, y: factsY, size: 9, font, color: MUTED })
    page.drawText(value, {
  ...
  const drawItemHeader = () => {
    ...
    page.drawText('ITEM', { x: columns.item, y, size: 8, font: bold, color: MUTED })
    page.drawText('QTY', { x: columns.qty, y, size: 8, font: bold, color: MUTED })
    page.drawText('UNIT', { x: columns.uom, y, size: 8, font: bold, color: MUTED })
  ...
  for (const line of doc.lines) {
    ...
    page.drawText(truncate(line.name, bold, 10, 240), {
      x: columns.item, y, size: 10, font: bold, color: INK,
    })
    page.drawText(String(line.quantity), { x: columns.qty, y, size: 10, font, color: INK })
    page.drawText(truncate(line.uomLabel, font, 10, 85), {
    ...
    page.drawText(sub.join(' · '), { x: columns.item, y, size: 8, font, color: MUTED })
    y -= 14
  }
  ...
  totalRow('Tax', money(doc.taxTotal, doc.currency))
  page.drawLine({
  ...
  for (const payment of doc.payments) {
    const label = [`Paid · ${methodLabel(payment.method)}`, payment.reference ? `#${payment.reference}` : null]
      .filter(Boolean)
      .join(' ')
...
async function renderThermal(doc: ReceiptDocument): Promise<Uint8Array> {
  ...
  for (const row of plan.rows) {
    ...
    if (row.right !== undefined) {
      page.drawText(row.text, { x: pad, y: y - row.size, size: row.size, font: face, color: row.color })
...
function planThermal(
  ...
  text('Tax', { right: money(doc.taxTotal, doc.currency), color: MUTED })
  text('TOTAL', { right: money(doc.total, doc.currency), size: 10, bold: true })

  for (const payment of doc.payments) {
    const label = [methodLabel(payment.method), payment.reference ? `#${payment.reference}` : null]
      .filter(Boolean)
      .join(' ')
    text(`Paid ${label}`, { right: money(payment.amount, doc.currency), color: MUTED })
  }
