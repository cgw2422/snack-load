...
export async function runInventoryReport(
  ...
  const raw = await reportQuery<RawRow>(
    ...
    Prisma.sql`
      WITH balances AS (
        SELECT b.product_id,
               SUM(b.quantity)::bigint AS on_hand,
               ...
               CASE WHEN SUM(b.quantity) > 0
                    THEN SUM(b.avg_unit_cost * b.quantity) / SUM(b.quantity)
      ...
      movement AS (
        SELECT l.product_id,
               ...
               COALESCE(SUM(
                 CASE WHEN t.type IN ('SALE', 'CUSTOMER_RETURN')
                        OR (t.type = 'REVERSAL' AND t.reference_type = 'Sale')
  ...
  const columns: ReportColumn[] = [
    ...
    { key: 'onHand', label: 'On hand', format: 'number', hint: 'Base units, all locations' },
  ...
  const notes = [
    'On hand is every location combined — warehouse and all trucks.',
  ...
  return {
    ...
    definition:
      'On hand is the current balance across every location, in base units, ' +
      'taken now rather than as of the end of the window. Value is those units at ' +
      'the moving-average cost of the goods — what they cost to buy, not what ' +
      'they will sell for. Sold is net units that left on a sale, less customer ' +
      'returns and less the reversals written when a sale is voided. Received is ' +
      'supplier receipts. Adjusted is damage, expiry, missing stock, samples and ' +
      'count corrections, netted together. Truck loads and transfers move stock ' +
